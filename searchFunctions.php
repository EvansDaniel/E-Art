...
function search($string,$con) {
  ...
  for($j=0; $j<$r->num_rows; $j++) {
    $q= "select itemId,imgPath,price,name,artistId from products where tagId1 = '" . 
  ...
  for($j=0; $j<$r->num_rows; $j++) {
    ...
    // echo "<br>";
    $q = "select fName from people where artistId={$artistId[$j]}"; 
    $res = $con->query($q);
    if(!$res) die($con->error);
    $artist = $res->fetch_array(MYSQLI_ASSOC);

    echo $output = "<li id=strval($j)> 
      <div>
        <div>
          <a href='http://www.yogakitty.com'><img src='{$imgPath[$j]}'>
          <a href='{$_SERVER['PHP_SELF']}?itemId={$itemId[$j]}'> 
          <button class='quicklook'>Quick Look</button></a>
        </div>
      <aside>
        
        <p id='title'>{$name[$j]}</p>
        <p id='artist'>by {$artist['fName']}</p>
